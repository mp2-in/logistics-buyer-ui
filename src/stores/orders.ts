...
interface State extends Attributes {
    ...
    createOrder: (token: string, billNumber: string, pickupOtp: string, storeId: string, drop: LocationAddress, amount: string, lspId: string | undefined,
    ...
    getPriceQuote: (token: string, storeId: string, drop: LocationAddress, orderAmount: number, callback: (success: boolean, quoteId: string, message?: string, priceQuotes?: PriceQuote[]) => void) => void
...
export const useOrdersStore = create<State>()((set, get) => ({
    ...
    createOrder: async (token, billNumber, pickupOtp, storeId, drop, amount, lspId, quoteId, itemId, readyToShip, callback) => {
        set(produce((state: State) => {
            ...
            Api('/webui/quotes', {
                ...
                .then(res => {
                    if (res.status === 1) {
                        ...
                        callback(true, res.quote_id, undefined, res.quotes)
                    } else {
                        set(produce((state: State) => {
                            state.activity.getPriceQuote = false
                        }))
